...
This document provides instructions for running **PdfLrt** in fully offline environments (without an active internet connection) on Windows, macOS, Linux, and iOS. It also details causes of possible failures and how to resolve such situations to ensure that PdfLrt can operate 100% TCP/IP network-independent.

---

## 1. Causes of Previous Offline Failures

During initial testing on a macOS machine with no internet connection, PdfLrt failed to operate, due to two distinct causes:
...
```
When offline, the browser could not resolve for download this package. The Web Worker crashed immediately on initialization, preventing the LiteRT LLM engine from loading.

**Resolution:**

The package must be served locally. The Go backend automatically downloads `tasks-genai.js` into the `wasm/` directory on its first run while online. `worker.js` and has been updated to load from the same-origin relative path:
...
In `sw.js`, the service worker static cache list (`urlsToCache`) included a remote stylesheet URL for Google Fonts:
...
Copy the zip/tarball file to the target machine (via home network, USB drive, etc.) and extract it to the target directory of choice.

### Step 3: Add Your Models

Ensure that:

1. The target PDF manuals are placed in the `./PdfDir` folder.
...
3. These set of actions registers the new PWA Service Worker, caches the local WASM files, and ensures a clean local execution.
...
3. **Remote Access over LAN (e.g. from an iPad)**:
   ...
   Because LAN IPs (e.g., `http://192.168.1.50:8080`) are insecure HTTP, macOS browsers will block WebGPU. If accessing a macOS server remotely, configure the remote device to treat the server address as secure (see iOS/Windows instructions below).
...
3. **Treating LAN Server as Secure Context**:
   ...
   - Relaunch Chrome. This allows WebGPU to run over insecure LAN connections.
...
2. **Bypassing LAN Secure Context constraints**:
   Because LAN IP connections are HTTP, Safari blocks WebGPU. You can bypass this restriction by adding the page to your Home Screen:
   - Open Safari and type in your host's local IP address (e.g., `http://192.168.1.15:8080`).
   - Tap the **Share** button in Safari and choose **Add to Home Screen**.
   - Open PdfLrt via the new PWA Home Screen icon. Standalone PWAs run in a sandboxed wrapper that automatically grants secure context privileges, enabling WebGPU access.
