...
*   **LiteRT-LM:** Built is built on top of Google's mobile optimization stack (TF Lite for AI Edge). It utilizes **XNNPack CPU acceleration**, whhich translates into a significantly faster CPU/Wasm execution speed on mobile devices, allowing the app to remain usable even on devices without GPU access.
...
*   LiteRT-LM is the official runtime optimized by Google to execute these features, achieving up to **2.2x faster decoding speeds** on mobile GPUs and CPUs while consuming substantially less memory than standard LLM architectures.
